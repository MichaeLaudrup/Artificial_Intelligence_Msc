...
def _render_progress_bar(done: int, total: int, width: int = 32) -> str:
...
def _print_progress(
    ...
    *,
    ...
    best_txt = f"{best_score:.4f}" if best_score > -1e8 else "-"
    line = (
        f"\r📈 Progreso [{bar}] {pct:6.2f}% ({done}/{total}) "
        f"| OK:{ok} FAIL:{fail} SKIP:{skip} | Best bAcc:{best_txt}"
    )
...
def sample_cfg():
    ...
    return {
        ...
        "focal_alpha_pos": random.choice(SEARCH_SPACE["focal_alpha_pos"]),
        ...
        "threshold_objective": random.choice(SEARCH_SPACE["threshold_objective"]),
    }
    ...
    for rec in good:
        ...
        out_list.append({
            ...
            "hyperparameters": {
                ...
                "focal_alpha_pos": cfg.get("focal_alpha_pos"),
                ...
                "threshold_objective": cfg.get("threshold_objective"),
            },
            "validation_metrics": {
...
def run_trial(cfg: dict, upto_week=5):
    ...
    run_cfg = {
        ...
        "focal_alpha_pos": cfg["focal_alpha_pos"],
...
def main(max_trials: int = MAX_TRIALS):
    ...
    _print_progress(0, max_trials, ok=n_success, fail=n_fail, skip=n_skip, best_score=best_score)
    ...
    for t in range(1, max_trials + 1):
        ...
        if cid in seen:
            n_skip += 1
            _print_progress(t, max_trials, ok=n_success, fail=n_fail, skip=n_skip, best_score=best_score)
            continue
        seen.add(cid)
        ...
        cfg_short = (
            ...
            f"γ={cfg['focal_gamma']} α+={cfg['focal_alpha_pos']} bs={cfg['batch_size']} obj={cfg['threshold_objective']}"
        )
        print(f"\n{'─'*70}")
    ...
    _print_progress(max_trials, max_trials, ok=n_success, fail=n_fail, skip=n_skip, best_score=best_score, force_newline=True)
