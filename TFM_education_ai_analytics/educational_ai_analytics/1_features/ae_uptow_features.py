...
class AEUptoWFeaturesBuilder:
    """
        Features acumulativas hasta W, normalizadas por curso-convocatoria.
    """
    ...
    activity_types: list[str] = field(default_factory=lambda: [
    ...
    activity_weights: dict[str, float] = field(default_factory=lambda: {
        "dataplus": 1.3,
        "dualpane": 0.7,
        "externalquiz": 1.3,
        "folder": 0.7,
    ...
    course_stats_: dict[int, dict] = field(init=False, default_factory=dict)

    def __post_init__(self):
        self.features_root_dir = Path(self.features_root_dir)

    @staticmethod
    def _students_index(df_students: pd.DataFrame) -> pd.DataFrame:
        stu = df_students
        if "unique_id" in stu.index.names and stu.index.is_unique:
            return stu
        if not stu["unique_id"].is_unique:
            stu = stu.drop_duplicates("unique_id")
        return stu.set_index("unique_id")

    @staticmethod
    def _course_series(stu: pd.DataFrame, index_uids: pd.Index) -> pd.Series:
        course = stu["code_module"].astype(str) + "_" + stu["code_presentation"].astype(str)
    ...
    def _weekly_clicks_by_type(
        ...
        inter["sum_click"] = pd.to_numeric(inter["sum_click"], errors="coerce").fillna(0.0).astype(float)
        inter["date"] = pd.to_numeric(inter["date"], errors="coerce").fillna(0.0).astype(float)
        inter["week"] = np.floor(inter["date"] / 7.0).astype(int)
        inter["activity_type"] = inter["activity_type"].astype(str).str.lower()

        pre = inter[inter["date"] < 0]
        prestart_clicks = pre.groupby("unique_id")["sum_click"].sum()
        prestart_clicks = prestart_clicks.reindex(index_uids).fillna(0.0)

        cur = inter[(inter["week"] >= 0) & (inter["week"] < maxW)].copy()
        cur = cur[cur["activity_type"].isin(set(self.activity_types))].copy()
        ...
        g = cur.groupby(["unique_id", "activity_type", "week"])["sum_click"].sum()
        m = g.unstack(fill_value=0.0)
        for w in range(maxW):
    ...
    def _assessments_uptoW(self, df_assessments: pd.DataFrame, index_uids: pd.Index, W: int) -> pd.DataFrame:
        ...
        if "date_submitted" not in asm.columns:
            return out

        sub = pd.to_numeric(asm["date_submitted"], errors="coerce")
        ...
        score = pd.to_numeric(asm.get("score", 0.0), errors="coerce").fillna(0.0).astype(float)
        deadline = pd.to_numeric(asm.get("date", np.nan), errors="coerce").astype(float)

        sub_sel = pd.to_numeric(asm["date_submitted"], errors="coerce").fillna(0.0).astype(float)
    ...
    def build_for_split(
        ...
        *,
        ...
        stu = self._students_index(df_students)
        index_uids = stu.index
        course = self._course_series(stu, index_uids)
        ...
        for W in windows:
            W = int(W)
            feat = {
                "current_week": pd.Series(float(W), index=index_uids)
            }
            ...
            if W not in self.course_stats_:
                raise ValueError(f"No se encuentran estadísticas AE UptoW para la ventana W={W}.")
            statsW = self.course_stats_[W]
